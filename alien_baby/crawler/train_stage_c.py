"""
train_stage_c.py — Stage C: train the zero-init residual vision head (ResidualVisionPolicy)
on top of the FROZEN blind crawler, by PPO reward. Vision can only ADD steering; the
proprioceptive gait is preserved by construction. macOS: vision => DummyVecEnv.

Usage:
  PYTHONPATH=<repo> python -u -m alien_baby.crawler.train_stage_c \
    --steps 1000000 --run-tag stage_c_v1
"""
import argparse
import pathlib
import pickle
...
from alien_baby.crawler.residual_vision_policy import ResidualVisionPolicy

RESULTS = pathlib.Path(__file__).parent.parent / "results"
XML = "alien_baby/crawler/mimo_crawler_pos_wide.xml"
BLIND = "crawl_ppo_blind_base"            # ckpt dir (holds vec_normalize.pkl)
BLIND_BEST = "crawl_ppo_blind_base_best"  # best_model.zip lives here
...
def load_base_norm():
    with open(RESULTS / BLIND / "vec_normalize.pkl", "rb") as f:
        vn = pickle.load(f)
...
def main():
    ...
    p.add_argument("--spawn-cone-deg", type=float, default=44.0)   # +/-22, matched to camera
    ...
    p.add_argument("--run-tag", default="stage_c_v1")
    p.add_argument("--device", default="mps")
    args = p.parse_args()

    tag = args.run_tag
    print(f"\n=== Stage C residual-vision training: {tag} ===")
    print(f"  steps={args.steps} n_envs={args.n_envs} cone=+/-{args.spawn_cone_deg/2:.0f} "
          f"device={args.device}")

    base_mean, base_std, base_clip = load_base_norm()
    train_env, eval_env = build_envs(args)

    model = PPO(
        ResidualVisionPolicy, train_env,
        ...
        policy_kwargs=dict(
            base_model_path=str(RESULTS / BLIND_BEST / "best_model.zip"),
            base_mean=base_mean, base_std=base_std, base_clip=base_clip,
        ),
    )
